package com.xiaou.pan.server.common.aspect;
...
public class CommonLoginAspect {
     ...
     * 切点表达式
     */
    private static final String POINT_CUT = "execution(* com.xiaou.pan.server.modules.*.controller..*(..))";
    ...
    public Object loginAuthAround(ProceedingJoinPoint proceedingJoinPoint) throws Throwable {
        if (checkNeedCheckLoginInfo(proceedingJoinPoint)) {
            ...
            if (!checkAndSaveUserId(request)) {
                log.warn("成功拦截到请求，URI为{},检测到用户未登录，将跳转到登陆页面", requestURI);
     ...
     *
     * @param proceedingJoinPoint
     * @return true 需要校验登陆信息 false 不需要
     */
